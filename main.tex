...
\begin{mdframed}[style=ejercicio-facil]
    \begin{ejercicio}
    \end{ejercicio}
    \noTi{Claudia pesa $60\, \kgf$, y viaja en un ascensor desde el piso 4 hasta la planta baja. Hallar el trabajo
    que realiza la fuerza que ejerce el piso del ascensor ("normal") sobre ella, en los siguientes tramos de $4\, \m$ de longitud cada uno:}
    \begin{enumerate}
        \item Arranque con aceleración constante, de $0,5\, \mpss$
        \item Descenso con velocidad constante, de $2\, \mps$
        \item Frenado con aceleración constante, de $0,5\, \mpss$
    \end{enumerate}
\end{mdframed}
...
\begin{mdframed}[style=ejercicio-facil]
    \begin{ejercicio}
    \end{ejercicio}
    \noTi{Un arquero tira hacia atrás la cuerda de su arco $0,4\, \m$ al ejercer una fuerza que aumenta
    uniformemente de cero a $230\, \N$.}
    \begin{enumerate}
...
\begin{mdframed}[style=ejercicio-facil]
    \begin{ejercicio}
    \end{ejercicio}
    \noTi{El bloque de $50\, \kg$ asciende por el plano inclinado de la figura y recorre $2\, \m$ sobre el mismo, con la
    fuerza horizontal constante $F_1$ aplicada de $600\, \N$. También actúa una fuerza de rozamiento de $100\,
    \N$. Hallar:}
    \begin{enumerate}
    ...
    \begin{center}
        \def\svgwidth{0.5\linewidth}
        \input{./images/trabajo-1.pdf_tex}
    \end{center}
\end{mdframed}
...
\begin{mdframed}[style=ejercicio-facil]
    \begin{ejercicio}
    \end{ejercicio}
    \noTi{Una partícula se somete a una fuerza $F_x$ que varía con la posición, como se muestra en la figura.
    Encontrar sin hacer cuentas, el trabajo total invertido por la fuerza.}
    \begin{center}
        \def\svgwidth{0.5\linewidth}
        \input{./images/trabajo-2.pdf_tex}
    \end{center}
\end{mdframed}
...
\begin{mdframed}[style=ejercicio-intermedio]
    \begin{ejercicio}
    \end{ejercicio}
    \noTi{Una fuerza $\vec{F}=(4 x \hat{x} + 3y \hat{y})\, \Npm$ actúa sobre un objeto mientras el objeto se mueve en la
    dirección $x$ desde el origen hasta $x = 5\, \m$. Hallar el trabajo invertido por la fuerza sobre el objeto.}
\end{mdframed}

\begin{mdframed}[style=ejercicio-intermedio]
    \begin{ejercicio}
    \end{ejercicio}
    \noTi{Se tiene un resorte cuya longitud sin carga es $0,8\, \m$, y su constante elástica es $500\, \Npm$. Dejando
    fijo un extremo, se lo estira hasta que su longitud es el doble de la original (Posición $A$), para luego
    comprimirlo hasta la mitad de su longitud natural (Posición $B$). Se pide:}
    \begin{enumerate}
        \item Graficar la componente de la fuerza que ejerce el resorte, en función de su elongación.
        \item Determinar el trabajo que realiza la fuerza elástica, al estirarlo desde la posición inicial hasta
        $A$.
        \item Hallar el trabajo realizado por la fuerza elástica entre las posiciones $A$ y $B$.
    \end{enumerate}
\end{mdframed}
...
\begin{mdframed}[style=ejercicio-intermedio]
    \begin{ejercicio}
    \end{ejercicio}
    \noTi{Un objeto de $3\, \kg$ tiene una velocidad de $(6 \hat{x}-2 \hat{y})\, \mps$.}
    \begin{enumerate}
        \item ¿Cuál es su energía cinética en ese momento?
        \item ¿Cuál es el trabajo neto invertido por el objeto si su velocidad cambia a $(8 \hat{x} + 4 \hat{y}\, \mps)$?
    \end{enumerate}
\end{mdframed}
...
\begin{mdframed}[style=ejercicio-dificil]
    \begin{ejercicio}
    \end{ejercicio}
    \noTi{Considerar un cuerpo de masa $m$ que cuelga de una cuerda de longitud $L$ y masa despreciable,
    cuyo otro extremo se halla fijo al punto $O$. Sobre el cuerpo actúa una fuerza $F= F_0 \cos{\theta}$ que
    produce su desplazamiento desde el punto $A$ hasta el punto $B$ (ver figura, $\theta$ es el ángulo con
    respecto a la dirección $OA$).}
    \begin{enumerate}
    ...
    \begin{center}
        \def\svgwidth{0.5\linewidth}
        \input{./images/trabajo-3.pdf_tex}
    \end{center}
\end{mdframed}
...
\begin{mdframed}[style=ejercicio-intermedio]
    \begin{ejercicio}
    \end{ejercicio}
    \noTi{El forzudo Igor levanta una pesa de $200\, \kg$ por encima de su cabeza, desde el suelo hasta una altura
de $2\, \m$.}
    \begin{enumerate}
...
\begin{mdframed}[style=ejercicio-dificil]
    \begin{ejercicio}
    \end{ejercicio}
    \noTi{Determinar el trabajo que realiza la fuerza de rozamiento que actúa sobre un cuerpo de $20\, \kg$, al
    desplazarlo desde el punto $A$ hasta el $B$ del esquema (vista desde arriba de las trayectorias) tirando
    de él con una cuerda paralelamente a la superficie horizontal donde está apoyado, si el coeficiente
    respectivo es $\mu_d= 0,4$.}
    \begin{enumerate}
        \item Por el camino directo ($1$)
        \item Pasando previamente por C (Camino $2$)
        \item Por la semicircunferencia ADB (Camino $3$)
        \item ¿Es conservativa la fuerza de rozamiento?
        \item Hallar el trabajo que realiza la fuerza que ejerce la cuerda en cada caso, suponiendo que el
        objeto se desplaza con velocidad de módulo constante.
    \end{enumerate}
    \begin{center}
        \def\svgwidth{0.5\linewidth}
        \input{./images/trabajo-4.pdf_tex}
    \end{center}
\end{mdframed}
...
\begin{mdframed}[style=ejercicio-intermedio]
    \begin{ejercicio}
    \end{ejercicio}
    \noTi{Una máquina eleva verticalmente una carga de $200\, \kg$ mediante una cuerda que se arrolla en un
...
\begin{mdframed}[style=ejercicio-intermedio]
    \begin{ejercicio}
    \end{ejercicio}
    \noTi{Una grúa iza verticalmente una caja de caudales de $400\, \kg$, que parte del reposo con aceleración
    constante durante $2\, \s$, hasta alcanzar una velocidad de $2\, \mps$; prosigue con ella durante $5\, \s$, para
    frenar luego y detenerse en otros $2\, \s$.}
    \begin{enumerate}
...
\begin{mdframed}[style=ejercicio-facil]
    \begin{ejercicio}
    \end{ejercicio}
    \noTi{La potencia del motor de un vehículo le alcanza para subir por una pendiente de $\ang{60}$ con una
    velocidad de $10\, \kmph$. Si subiera por otra pendiente de $\ang{30}$, sin modificar la velocidad, ¿en qué porcentaje disminuiría la potencia?}
    \begin{enumerate}
        \item $13\%$
        \item $30\%$
        \item $42\%$
...
\begin{mdframed}[style=ejercicio-facil]
    \begin{ejercicio}
    \end{ejercicio}
    \noTi{Sabiendo que la energía eléctrica cuesta $\$0,08\, \/\kWh$ ($8$ centavos de pesos por cada kWh de energía),
    mantener encendida una bombita de $100\, \W$ durante todo el día un mes completo ($30$ días) cuesta:}
    \begin{enumerate}
        \item $\$ 57,6$
        \item $\$ 12,8$
        \item $\$ 5,76$
